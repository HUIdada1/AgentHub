...
3. **第一次**同步会在 `%TEMP%` 建沙盒、启动 LS、跑 RPC，约 1-3 分钟（取决于 .pb 数量）；
4. 完成后自动清理沙盒与 LS 进程；**之后每次同步不再卡**——指纹（全部老 .pb 的文件名:大小:mtime 哈希）未变就秒返回，不做任何重活。

**增量语义（解决「每次同步都卡 100 秒」）：**
- 适配器把每次恢复完成的「完成标记 + 指纹」写进平台库 meta 表；之后只要老 .pb 集合没变化就跳过；
- 指纹变化（新增/更新过 .pb）→ 自动再跑一次完整恢复（幂等键不变，`INSERT OR REPLACE` 只覆盖不重复入账）；
- 你想强制重跑：删除平台库 meta 里的 `antigravity-legacy:state` 键，或直接「重置同步数据」（锚点=0 会自然触发全量）。

边界与降级：
- 没找到 `language_server.exe`（未安装 Antigravity）→ 跳过，不报错；
- 老 .pb 目录不存在 → 跳过；
- 单个会话 RPC 失败（比如 implicit 的轨迹）→ 跳过该会话，其他不受影响；**只有 0 失败才写完成标记**，否则下次同步自动重试；
- LS 端口动态探测（127.0.0.1 随机空闲端口），同步前按命令行精确清理残留 LS 进程，杜绝「残留进程占住固定端口、干等 60 秒」；
- LS 启动即崩立即放弃（不等满 25 秒），记错误日志，不影响其他源；
...
输出示例（本机 200 个 .pb，第二次起秒返回）：

```
=== Antigravity 老数据恢复适配器自测 ===
extract(sinceMs=0) 完成，耗时 6.4s，共 3008 条记录
...
--- 按月份 ---
  2026-04: 354 条，输入 1,963,310 / 输出 333,744 / 思考 138,756
  2026-05: 883 条，输入 8,170,726 / 输出 1,232,639 / 思考 434,335
  2026-07: 1771 条，输入 21,846,242 / 输出 2,827,022 / 思考 1,444,240

extract(sinceMs=1, 指纹未变) 耗时 0.01s，产出 0 条（应为 0）
  ✓ 增量语义正确（只在数据变化时跑一次完整恢复）
```
